...
async function connect() {
  ...
  await Promise.all([
    ...
    database.collection('permissions').createIndex({ id: 1 }, { unique: true }),
    ...
    database.collection('customers').createIndex({ created_at: -1 }),
    database.collection('products').createIndex({ id: 1 }, { unique: true }),
    database.collection('products').createIndex({ productname: 1 }),
    database.collection('invoices').createIndex({ id: 1 }, { unique: true }),
    database.collection('invoices').createIndex({ invoice_no: 1 }, { unique: true }),
    database.collection('invoices').createIndex({ customer_id: 1 }),
    database.collection('invoices').createIndex({ is_deleted: 1, invoice_date: -1, id: -1 }),
